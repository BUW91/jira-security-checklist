...
import ForgeReconciler, { Text, Box, Heading, DynamicTable, Button, Toggle, Label, Textfield } from '@forge/react';
...
const App = () => {
  ...
  const formatRows = (list) => {
    ...
    // Add row for adding a new item
    rows.push({
      key: 'add-item',
      cells: [
        { content: <Text content="Add new item" /> },
        {
          content: (
            <Button
              appearance='subtle'
    ...
    {templateLists.map((list, index) => (
      <Box key={index}>
        ...
        <DynamicTable
          ...
          isRankable
          onRankEnd={({ sourceIndex, destinationIndex }) => handleRankEnd(sourceIndex, destinationIndex, list.id)}
        ></DynamicTable>
      </Box>
    ))}
  </Box>
